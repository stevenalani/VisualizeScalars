...
namespace VisualizeScalars.DataQuery
{
    ...
    public class EarthdataDownloader
    {
        ...
        private readonly CredentialCache cache = new CredentialCache();
        ...
        /// <param name="granuleCount">
        ///     Anzahl der zu downloadenden Granulare. 0 = Alle Granulare (Vorsicht kann im GB bereich
        ///     liegen)
        /// </param>
        public void RegisterDataSource(string key, string conceptId, string fileFormat, int granuleCount = 0)
        {
            if (!DataSources.ContainsKey(key)) DataSources.Add(key, new[] {conceptId, fileFormat});

            if (GranulesToLoad.ContainsKey(key)) GranulesToLoad.Add(key, granuleCount);
        }
        ...
        public void DownloadDataSets(double latitudeSouth, double longitudeWest)
        {
            ...
            var spatialParam =
                $"&point={longitudeWest.ToString(CultureInfo.InvariantCulture)},{latitudeSouth.ToString(CultureInfo.InvariantCulture)}";
            foreach (var key in keys) DownloadDataSet(key, spatialParam);
        }
        ...
        public void DownloadDataSets(double latitudeSouth, double longitudeWest, double latitudeNorth,
            double longitudeEast)
        {
            ...
            var spatialParam =
                $"&bounding_box={latitudeSouth.ToString(CultureInfo.InvariantCulture)},{longitudeWest.ToString(CultureInfo.InvariantCulture)},{latitudeNorth.ToString(CultureInfo.InvariantCulture)},{longitudeEast.ToString(CultureInfo.InvariantCulture)}";
            var keys = DataSources.Keys.Select(key => key.ToString());
            foreach (var key in keys) DownloadDataSet(key, spatialParam);
        }
        ...
        {
            var spatialParam =
                $"&point={latitudeSouth.ToString(CultureInfo.InvariantCulture)},{longitudeWest.ToString(CultureInfo.InvariantCulture)}";
        ...
        /// <param name="latitudeNorth">Oben Longitude</param>
        /// <param name="longitudeEast">Rechts Latitude</param>
        public void DownloadDataSet(string key, double latitudeSouth, double longitudeWest, double latitudeNorth,
            double longitudeEast)
        {
            if (latitudeSouth > latitudeNorth)
            {
        ...
        private void DownloadDataSet(string key, string spatialParam)
        {
            ...
            var url = "https://cmr.earthdata.nasa.gov/search/granules.json?concept_id=" + DataSources[key][0];
            url += spatialParam;
            ...
            try
            {
                ...
                if (granuleCount != 0)
                    dataLinks = dataLinks.Take(granuleCount);
        ...
        public void readDatFiles()
        {
            ...
            var profileAvailableWaterCap = getWaterCapacity(files.First(x => x.Contains("pawc")));
        }

        private ProfileAvailableWaterCapacityInformation getWaterCapacity(string filename)
        {
            var streamReader = new StreamReader(filename, Encoding.ASCII);
            var pwacInfo = new ProfileAvailableWaterCapacityInformation();
            var line = "";
            var lineNo = 1;
            var coordinate = new Coordinate(-180.0, -56.5000);
    ...
    public class ProfileAvailableWaterCapacityInformation
    {
        ...
        public double CellSize => Headers.ContainsKey("cellsize")
            ? Convert.ToDouble(Headers["cellsize"], CultureInfo.InvariantCulture)
            : double.NaN;

        public double FirstCellX => Headers.ContainsKey("xllcorner")
            ? Convert.ToDouble(Headers["xllcorner"], CultureInfo.InvariantCulture)
            : double.NaN;

        public double FirstCellY => Headers.ContainsKey("yllcorner")
            ? Convert.ToDouble(Headers["yllcorner"], CultureInfo.InvariantCulture)
            : double.NaN;

        public double NoDataValue => Headers.ContainsKey("NODATA_value")
            ? Convert.ToDouble(Headers["NODATA_value"], CultureInfo.InvariantCulture)
            : double.NaN;
    }
