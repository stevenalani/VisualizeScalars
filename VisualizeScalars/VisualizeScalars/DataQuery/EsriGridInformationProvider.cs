...
namespace VisualizeScalars.DataQuery
{
    public class EsriGridInformationProvider
    {
        ...
        public double CellSize => _headers.ContainsKey("cellsize") ? Convert.ToDouble(_headers["cellsize"], CultureInfo.InvariantCulture) : Double.NaN;
        public double LastCellX => _headers.ContainsKey("xllcorner") ? Convert.ToDouble(_headers["xllcorner"], CultureInfo.InvariantCulture) : Double.NaN;
        public double LastCellY => _headers.ContainsKey("yllcorner") ? Convert.ToDouble(_headers["yllcorner"], CultureInfo.InvariantCulture) : Double.NaN;
        public double FirstCellX => LastCellX;
        public double FirstCellY => Math.Ceiling(LastCellY + Rows * CellSize);
        public double NoDataValue => _headers.ContainsKey("NODATA_value") ? Convert.ToDouble(_headers["NODATA_value"], CultureInfo.InvariantCulture) : Double.NaN;
        ...
        {
            if (latitude < LastCellY || latitude > FirstCellY)
            {
                return -1;
            }
        ...
        {
            if (longitude < LastCellX || longitude > Math.Abs(FirstCellX))
            {
                return -1;
            }
        ...
        public double[,] InformationForArea(double latitudeSouth, double longitudeWest, double latitudeNorth, double longitudeEast, double cellSize)
        {
            int indexX = 0, indexY = 0;
            var cellCountX = (int)((longitudeEast - longitudeWest) / cellSize) + 1;
            var cellCountY = (int)((latitudeNorth - latitudeSouth) / cellSize) + 1;
            double[,] result = new double[cellCountX,cellCountY];
            FileStream fs = new FileStream(ConvertedDataFile, FileMode.Open, FileAccess.Read);
            BinaryReader reader = new BinaryReader(fs);
            for (double latitude = latitudeSouth; latitude < latitudeNorth; latitude += cellSize)
            {
                
                for (double longitude = longitudeWest; longitude < longitudeEast; longitude += cellSize)
                {
                    var row = LatToRow(latitude); // row excel string = row+1!
                    var col = LngToCol(longitude);// col excel string = col+1!
        ...
        {
            long position = (row * Columns  + col) * sizeof(float);
            FileStream fs = new FileStream(ConvertedDataFile, FileMode.Open, FileAccess.Read) { Position = position };
            BinaryReader reader = new BinaryReader(fs);
